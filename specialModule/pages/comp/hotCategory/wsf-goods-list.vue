<template>
	...
	<view v-if="goodsList.length>0">
		<view v-if="false" class="applet-template-recommend-title">
			 <image
      ...
      ></image>
		...
		<view v-if="goodsListStyle == 'column'">
			<view class="list-goods">
				<template v-for="(item, index) in goodsList">
					<view @click="$emit('goodsClick', item)" :key="index">
						<column-goods :item="item"></column-goods>
					</view>
				</template>
			</view>
		</view>
		<!-- 瀑布流样式 -->
		<view v-else-if="goodsListStyle == 'waterfall'">
			<u-waterfall v-model="goodsList" ref="uWaterfall" add-time="0">
				<template v-slot:left="{ leftList }">
					<view v-for="(item, index) in leftList" :key="index" @click="$emit('goodsClick', item)">
						...
						<block v-else>
							<waterfall-goods :item="item" type="left"></waterfall-goods>
						</block>
					</view>
				</template>
				...
				<view v-else class="waterfall-goods" @click="$emit('goodsClick', item)">
					...
					<view class="waterfall-goods-info common-goods-info">
						...
						<view class="waterfall-goods-price">
							...
							<text class="waterfall-original-price" v-if="item.afterCouponPrice">¥{{ item.finalPrice }}</text>
							<text class="waterfall-old-price" v-else-if="item.finalPrice != item.price">¥{{ item.price }}</text>
						</view>
