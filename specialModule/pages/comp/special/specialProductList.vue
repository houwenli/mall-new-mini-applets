<template>
  <view class="item">
    <!-- <view v-if="item.status == '0'" class="add-cart-icon" @click.stop="handleAddCart(item)">
    ...
    <view class="img-txt">
      ...
      <!-- <view class="txt" v-if="item.status != 0">
        <text v-if="item.status==1">下架</text>
        <text v-if="item.status==2">无货</text>
        <text v-if="item.status==3">售罄</text>
        <text v-if="item.status==4">失效</text>
      </view> -->
    </view>
    <view class="column-goods-info">
      <view class="column-goods-info-top">
        <view class="name" :class="[(item.status != 0) ? 'disable-status' : '']">
          {{item.name}}
        </view>
        ...
        <view class="column-goods-buy">
          <block v-if="item.skuLable.couponLable && afterCouponPrice && false">
          ...
          <view class="performance-mark" v-if="item.skuLable && item.skuLable.performanceScoreLabel && item.skuLable.performanceScoreLabel.performanceScore > 0">
            <image class="icon" :src="performanceIcon" /> 绩效分: {{ item.skuLable.performanceScoreLabel.performanceScore }}
          </view>
        </view>

      </view>
      <view class="column-goods-info-bottom" v-if="item.status!=1 && item.status!=4">
        <!-- 商品价格 -->
        ...
        <view class="column-goods-price" v-if="false">
          <text class="column-after-coupon-text" v-if="item.sourceType != 2 && item.skuLable.couponLable && afterCouponPrice">券后</text>
          <text :class="[item.sku.status != '0' ? 'disable-status' : '']">¥</text>
          <text class="column-goods-price-num" v-if="item.sourceType != 2 && item.skuLable.couponLable && afterCouponPrice">{{ afterCouponPrice | priceInt }}</text>
          <text class="column-goods-price-num" :class="[item.sku.status != '0' ? 'disable-status' : '']" v-else>{{ item.salePrice | priceInt }}</text>
          <text v-if="item.sourceType != 2 && item.skuLable.couponLable && afterCouponPrice">{{afterCouponPrice | priceFloat}}</text>
          ...
          <view class="column-old-price" v-else-if="item.salePrice != item.price">
            <text class="old-price-small">¥</text>
            <text class="old-price">{{ item.price | priceInt }}</text>
            <text class="old-price-small">{{ item.price | priceFloat}}</text>
          </view>
