<template>
    <view class="back-top">
        ...
        <view class="box-top" :style="{ background: topSearch ? '#ffffff' : '', height: headerIconHeight + 'px' }">           
            <view class="icon-left" @click="goIndex" v-if="isShare"><img src="https://wsjc-web.wsecar.com/wsf-mall/skuDetail/homePage.png" alt=""></view>
            <view class="icon-left back-img" @click="goUponelevel" v-else-if="!isShare"><img :src="`${smartImg}/detail/2023-09-25/icon-left.png`" alt=""></view>
            <view class="icon-left more" @click="goOverlay" v-if="isShowSearch"><img :src="`${smartImg}/detail/icon-more.png`" alt=""></view>
        ...
        <view v-if="showOverlay && isShowSearch" class="indicator_">
...
export default {
    ...
    methods: {
        ...
        goUponelevel() {
            if(this.goBack){
                this.$wsf.go(this.goBack, {}, "redirectTo")
            }else{
                uni.navigateBack({
                    delta: 1,
                });
