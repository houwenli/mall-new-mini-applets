<template>
	<view class="u-upload u-flex" v-if="!disabled">
		<view
			v-if="showUploadList"
			class="u-list-item u-preview-wrap"
			:class="{'u-list-item-margin-4': marginType == 4,'u-list-item-margin-default': marginType == 0}"
		...
		>
			<view v-if="deletable" class="u-delete-icon u-flex u-row-center u-col-center" @tap.stop="deleteItem(index)">
				<image src="https://wsjc-web-1301582899.cos.ap-guangzhou.myqcloud.com/wsf-mall/order/shouhou_image_upload_delect.png" style="height: 32rpx;width: 32rpx;" mode=""></image>
			</view>
			<u-line-progress
...
export default {
	...
	methods: {
		...
		async uploadFile(index = 0) {
			...
			const task = uni.uploadFile({
				...
				success: res => {
					...
					if (![200, 201, 204].includes(res.statusCode) || data.code !== 200) {
        ...
        activeconfirm(){
            this.handlerDeleteItem(this.delectIndex);
            this.activeCancel()
        },
		// 删除一个图片
		deleteItem(index) {
...
.u-list-item-margin-4 {
	margin:8rpx 24rpx 16rpx 0rpx;
    &:nth-child(4n-1) {
		margin-right: 22rpx;
	}
	&:nth-child(4n) {
