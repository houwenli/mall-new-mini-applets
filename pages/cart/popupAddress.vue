<!--
 * @Author: DaiYu
 * @Date: 2022-12-30 17:56:23
 * @LastEditors: DaiYu
 * @LastEditTime: 2023-01-02 09:14:31
 * @FilePath: \productModule\pages\skuDetail\components\popupAddress.vue
 * @Description: file content
-->
<template>
	<u-popup v-model="value" mode="bottom" @close="close" @open="openPopup" :safe-area-inset-bottom="true"
		border-radius="20" height="60%">
		<view class="addresspop">
			...
			<scroll-view scroll-y="true" class="addresspop-content">
				<!-- <view class="addresspop-location">
					<view class="addresspop-location__icon"></view>
					<view class="addresspop-location__content">
						<view class="addresspop-location__content-top">
							<view class="addresspop-location__desc">当前定位</view>
							<view class="addresspop-location__address-detail">光雅园美食街四巷三号</view>
						</view>
						<view class="addresspop-location__address">广东深圳市南山区南头街道</view>
					</view>
					<view class="addresspop-location__retry" @click="getLocation()">重新定位</view>
				</view> -->
				...
				uni.getLocation({
					...
					success: (res) => {
						qqMap.reverseGeocoder({
							...
							success:  (res) => {
								let {city, district, province, street_number} = res.result.address_component
								// this.streeAdress = street_number
								// this.proCityDis = province + city + district
								console.log(res, "====qqMap", street_number , province + city + district );
							},
							fail: function (error) {
								console.error(error);
							},
