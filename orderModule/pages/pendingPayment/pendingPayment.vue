<template>
  <view class="pending-payment">
    ...
    <back-top 
      ...
      :backTopStyle="{background: 'linear-gradient(225deg, #FF5F2E 0%, #FF0A35 100%)'}"
      >
      <template #title-diy>
    ...
    <view class="content">
      ...
      >
        ...
        <!-- 等待收货 -->
        <view class="txt" v-if="(status == 2 || status == 3) && titleDesc">{{titleDesc}}</view>
        <!-- 已取消 -->
        <!-- <view v-else style="height: 8rpx"></view> -->
        <!-- cancelStatus 1:用户自行取消 2: 定时任务取消（超时） -->
        <view class="txt" v-if="status == 5 && orderDetailData.cancelStatus == 2">
          取消原因：超时未支付
        </view>
        ...
        <!-- <view class="go-pay-btn" v-else-if="status == 6 || status == 7" @click="buyAgain()">再次购买</view> -->
      </view>
    </view>
    <view class="address-info">
      ...
      <view
        class="address-info-item flex-column address-info-logistics"
      ...
      >
        ...
        <view class="bottom u-flex">
          ...
          <image class="right-icon" src="https://wsjc-web-1301582899.cos.ap-guangzhou.myqcloud.com/wsf-mall/order/logistics-new.png"></image>
        </view>
      </view>
      ...
      >
        ...
        <view class="bottom u-flex">
          <view class="logistics">
            ...
            >
              <view class="top-title">
                <image src="https://wsjc-web-1301582899.cos.ap-guangzhou.myqcloud.com/wsf-mall/order/receipt-new.png"></image>
                <block v-if="status == 2">
                  <text class="txt">您提交了订单，请等待发货</text>
                </block>
                <block v-if="status == 6">
                  <text class="txt">您的订单已取消!</text>
                </block>
                <block v-if="(status == 3 || status == 7) && orderDetailData.kdniaoTrackVO">
                  <text class="txt">{{
          ...
          <image v-if="status == 3 || status == 2"  class="right-icon" src="https://wsjc-web-1301582899.cos.ap-guangzhou.myqcloud.com/wsf-mall/order/logistics-new.png"></image>
        </view>
      </view>
      <!--end 物流 -->
      ...
      <view
        class="address-info-item flex-column"
      >
        <view class="top">
        </view>
        <view class="bottom u-flex">
          <image
            src="https://wsjc-web-1301582899.cos.ap-guangzhou.myqcloud.com/wsf-mall/order/detail-location.png"
          ...
          <view class="address">
            ...
            <view class="address-detail">{{ site }}</view>
          </view>
          <!-- 暂时隐藏,后期看需求加上 -->
          <view class="modification" v-if="orderDetailData.status == '1' && orderDetailData.receiptUpdateNum < 1" @click="toaddressList()">
    ...
    <!-- 商品信息 -->
    <!-- 修改为大订单 -->
    <view v-if="orderDetailData.orderDetailVos && orderDetailData.orderDetailVos.length > 0">
      <view v-for="(bigOrder, bigOrderIndex) in orderDetailData.orderDetailVos" :key="bigOrder.id">
        <view class="merchandise-news" v-if="bigOrder.orderSkuWithMarketings && bigOrder.orderSkuWithMarketings.length > 0">
          <view class="title">共计{{bigOrder.orderSkuWithMarketings.length}}件商品</view>
          ...
          <view
            v-for="(item, index) in bigOrder.orderSkuWithMarketingsCopy"
            :key="index"
          >
            <view class="shop-container">
              <view class="shop-info" @click="goToSku(item.skuId,bigOrder)">
              <!--start 虚拟商品标签 -->
              ...
              <view class="txt-info">
                <view class="txt-info-content">
                  <view class="txt-1 u-line-2">{{ item.skuName }}</view>
                  <view class="txt-2">
                    <view class="sku-specs u-line-1">
                      <text>{{ "数量: " + item.num }}{{ item.skuSpecs }}</text>
                    </view>
                  </view>
                  <view class="tag-part">
                  ...
                  <view class="performance" v-if="item.performanceScore && item.performanceScore > 0">
                    <img src="https://wsjc-web.wsecar.com/wsf-mall/skuDetail/icon-performance.png" alt="">
                    <view class="num-pannel">绩效分:<text class="num">{{ item.performanceScore}}</text></view>
                  </view>
                </view>
                <view class="money-shoppingTrolley">
                  <view>
                    <!-- 添加福气值逻辑 -->
                    <view class="fqz-price" v-if="item.isFqzGoods == 1 && item.integralPoint > 0">
                      <text class="fqz-num" ><text class="special font-din">{{item.integralPoint}}</text>福气值+{{item.deductionAfterPrice}}元</text>
                    </view>
                    ...
                    <view class="unitPrice-valueCardPrice" v-if="!(titleName === '等待付款')">
                      <view class="money" v-if="bigOrder.valueCardPrice">
                        <text class="txt-1">礼品卡抵扣 </text>
                        <text class="txt-2">￥{{ priceFilter(item.valueCardPrice) }}</text>
                      </view>
                      <!-- <text style="color:#999;font-size:24rpx" v-if="bigOrder.status != 1 && item.skuPrice != item.unitPrice">实付单价</text><text class="reality-price" v-if="bigOrder.status != 1 && item.skuPrice != item.unitPrice">￥{{ item.unitPrice.toFixed(2) }}</text> -->
                    </view>
              ...
              <!-- <view class="shop-button-column">
                <view class="common-btn" v-if=" item.isDelivery == 1 && (bigOrder.status == 2 || bigOrder.status == 3) && item.orderType != 7" @tap="goToTrace(item, 2)">查看物流</view>
                <view class="common-btn" v-if="item.refundStatus == 1" @tap="goRefundServiceDetail(item)">退款中</view>
                <view class="common-btn" v-if="item.refundStatus == 2 && bigOrder.oldRefund != 1" @tap="goRefundServiceDetail(item)">退款完成</view>
                <view class="common-btn" v-if="item.refundStatus == 3" @tap="goRefundServiceDetail(item)">退款失败</view>
                <view class="common-btn" v-if="bigOrder.canRefund && (item.refundStatus == 0 || item.refundStatus == 3)&&bigOrder.orderType!=10" @tap="goToRefund(item)">申请退款</view>
                <view class="common-btn light-btn" v-if="status != 1" @click.stop="handleAddCart(item)">加购物车</view>
              </view> -->

              <!-- @tap="skuBtnClick(btnItem)" -->
              <view class="shop-button-column" v-if="item.btnArr && item.btnArr.length > 0">
            ...
            >
              <block
                ...
                :key="giftIndex"
              >
                <view class="shop-info" :key="index" @click="goToSku(gift.skuId,bigOrder)">
                  <view class="img"
                    ><image
                      :src="gift.url"
              ...
              <!--start 虚拟商品不需要显示-->
              <!-- <view
              v-if="bigOrder.orderType != 7"
              class="btn-box"
              :class="{
                'u-margin-bottom-30':
                  index < bigOrder.orderSkuWithMarketings.length - 1,
              }"
            >
              <view
                class="common_btn"
          ...
          <view class="sku-more-btn" v-if="bigOrder.orderSkuWithMarketings.length > 3" @click="isHasMore(bigOrderIndex)">
            <image v-if="bigOrder.hasMore" src="https://wsjc-web.wsecar.com/wsf-mall/skuDetail/icon-btn-btom.png"></image>
            <image v-else src="https://wsjc-web.wsecar.com/wsf-mall/skuDetail/icon-btn-top.png"></image>
          </view>
          <view class="sku-remark" v-if="bigOrder.remark">
            <view class="sku-remark__desc">买家留言</view>
            <view class="sku-remark__detail">{{ bigOrder.remark }}</view>
          </view>
          <!-- 联系客服 -->
          <view class="sku-service" @click="contactService(bigOrder)">
            <image class="sku-service__img" src="https://wsjc-web-1301582899.cos.ap-guangzhou.myqcloud.com/wsf-mall/order/service_new.png"></image>
            联系客服
          </view>
        </view>
      </view>
    ...
    <view class="order">
      <view class="item order-no">
        ...
        <view v-if="from=='smartApp'">
          <text class="txt-2">{{ orderDetailData.orderNo }}</text>
          <text class="order-copy"  @click="setClipboardData(orderDetailData.orderNo)">复制</text>
        </view>
        <view v-else>
          <!-- 已取消或者待支付 -->
          <block v-if="orderDetailData.status == '1' || orderDetailData.status == '5'">
            <text class="txt-2">{{ orderDetailData.masterOrderCode }}</text>
            <text class="order-copy"  @click="setClipboardData(orderDetailData.masterOrderCode)">复制</text>
          </block>
          <block v-else>
            <text class="txt-2">{{ orderDetailData.orderCode }}</text>
            <text class="order-copy"  @click="setClipboardData(orderDetailData.orderCode)">复制</text>
          </block>
        </view>
      </view>
      <view class="item order-no">
      ...
      <template v-if="status != 1">
        ...
        <template v-if="status != 5">
          ...
          >
            ...
            <text class="txt-2" v-if="!orderDetailData.mixedPayChannelName">/</text>
            <text class="txt-2" v-else>{{ orderDetailData.mixedPayChannelName }}</text>
          </view>
          <view class="item order-no">
          ...
          >
            ...
            <view v-if="orderDetailData.orderAndLogisticsList && orderDetailData.orderAndLogisticsList.length > 0">
              <view class="txt-2" v-for="(item, index) in orderDetailData.orderAndLogisticsList" :key="index">{{ item.logisticsCompany }} {{ item.waybillCode }}</view>
            </view>
            <view v-else>
              <view class="txt-2">快递运输</view>
            </view>
          </view>
          <!--end 虚拟商品无需显示-->
        </template>
        ...
        <view v-if="orderDetailData.orderType == 7">
          ...
          <view class="item order-no camilo-module">
    ...
    <view class="order">
        <view class="order-no">
          <text class="txt-1">商品总额</text>
          <text class="txt-2"
            >￥{{ priceFilter(orderDetailData.originalPrice) }}</text
          >
        </view>
        <view class="order-no">
          <text class="txt-1">运费</text>
         ...
         <view class="order-no" v-if="orderDetailData.orderTotalIntegralPoint">
          <text class="txt-1">福气值</text>
          <text class="txt-2" style="color: #FF0A35"
            >-{{ orderDetailData.orderTotalIntegralPoint }}</text
          >
        </view>

        <view class="order-no" v-if="orderDetailData.couponPrice">
          <text class="txt-1">店铺优惠券</text>
          <text class="txt-2"
            >-￥{{ orderDetailData.couponPrice }}</text
          >
        </view>
				<view class="order-no" v-if="orderDetailData.platformCouponPrice">
				  <text class="txt-1">平台优惠券</text>
				  <text class="txt-2"
				    >-￥{{ orderDetailData.platformCouponPrice }}</text
				  >
				</view>
        <view class="order-no" v-if="orderDetailData.valueCardPrice">
				  <text class="txt-1">礼品卡抵扣</text>
          <text class="txt-2"
            >-￥{{ priceFilter(orderDetailData.valueCardPrice) }}</text
          >
				</view>
        <view class="order-no" v-if="orderDetailData.activityPrice">
				  <text class="txt-1">活动</text>
				  <text class="txt-2 txt-red"
				    >-￥{{ priceFilter(orderDetailData.activityPrice) }}</text
				  >
				</view>
        <view class="order-no" v-if="orderDetailData.retailerCouponPrice">
				  <text class="txt-1">优惠券</text>
				  <text class="txt-2 txt-red"
				    >-￥{{ priceFilter(orderDetailData.retailerCouponPrice) }}</text
				  >
				</view>
        <view class="order-no" v-if="orderDetailData.performanceScore">
				  <text class="txt-1">绩效分</text>
				  <text class="txt-2 txt-red"
				    >{{ orderDetailData.performanceScore }}</text
				  >
				</view>
        <view class="payment">
          <text class="txt-1">{{ orderDetailData.status == 1 || orderDetailData.status == 5 ? '应付款' : '实付款' }}：</text>
          <text class="txt-2">￥</text>
          <text class="txt-3">{{ priceFilter(orderDetailData.price) }}</text>
        </view>
    </view>
    ...
    <!-- 底部按钮 -->
    <view class="bottom-btn" style="height: 96rpx" :class="{ 'needButtomSpace': isNeedBottomSpace }"></view>
...
export default {
  ...
  data() {
    return {
      from: '', // 从什么页面进入
      orderStatusIconDict: {
        1: this.$oss + 'wsf-mall/order/time-flag-xcx.png',
				2: this.$oss + 'wsf-mall/order/time-flag-xcx.png',
				3: this.$oss + 'wsf-mall/order/time-flag-xcx.png',
				4: this.$oss + 'wsf-mall/order/finish-flag-xcx.png',
				5: this.$oss + 'wsf-mall/order/cancel-flag-xcx.png',
				6: this.$oss + 'wsf-mall/order/cancel-flag-xcx.png',
				7: this.$oss + 'wsf-mall/order/cancel-flag-xcx.png',
      },
  ...
  methods: {
        ...
        paySceneType: this.orderDetailData.orderType * 1 === 14 ? 102 : 101 //102集采订单 101电商订单
      });
			}
		},
		//合并支付/取消订单 取消按钮
    ...
    filterGiftSku(giftskus, price, name) {
      ...
      return tempArr.length > 0 ? tempArr[0].giftSkuInfos : [];
		...
		onBackBtnClick() {
            // 从服务通知进来返回商城首页
            if(this.intoType === 'subscribeMessage'){
                uni.switchTab({
        ...
        if (this.status == 1 && this.countDown < 0) {
    ...
    // 自动取消订单
    autoSubmit(data) {
      let orderIdList = this.orderDetailData.orderDetailVos.map(item => item.id);
      let params = {
        orderIdList,
        addCart: data.isAddCart,
        reason: data.reason
      };
    ...
    //加购
    handleAddCart(params,bigOrder) {
      console.log(params, "加入购物车");
      let { num, skuId } = params;
      let data = { num, skuId };
      data.sourceType = bigOrder.sourceType
      data.storeId = bigOrder.storeId
      this.$u.api.AddShoppingCart(data).then((res) => {
        ...
        let data = err.data || {}
        this.showToastError(data && data.msg || '请稍后重试');
      })
    },
    // 商品跳转
    goodsClick(e) {
    ...
    // 前往商品详情
    goToSku(id,bigOrder) {
        if(this.orderDetailData.orderType * 1 === 14){
            // 集采订单--跳转集采商详
            uni.setStorageSync("enterpriseAgencyId", this.orderDetailData.enterpriseId);
            this.$wsf.go("collectionDetail", { skuId: id });
            return
        }
      this.$wsf.go("SkuDetail", { skuId: id, sourceType: bigOrder.sourceType, storeIdNumber: bigOrder.storeId  });
    },
    // 联系客服
    // serviceTel() {
    ...
    goToPay() {
          ...
          this.$wsf.go("Pay", {
            ...
            orderCode: this.orderDetailData.masterOrderCode,
            businessType: 100,
            paySceneType: this.orderDetailData.orderType * 1 === 14 ? 102 : 101 //102集采订单 101电商订单
          });
				});
			} else {
        this.$refs.orderSecondConfirm.orderOperation(this.orderDetailData, '去支付')
			}
    },
    ...
    // 申请退款
    goToRefund(item) {
      let {skuImage, skuName, price, num, id, skuPrice, skuSpecs} = item
    ...
    // 再次购买
    buyAgain() {
      var bigOrderList = this.orderDetailData.orderDetailVos || []
          ...
          if (list && list.length) {
            list.forEach((n) => {
              let skuObj = {
                ...
                recommendToken: null,
                num: n.num,
                // orderType 14 是集采订单 集采sourceType传5
                sourceType: this.orderDetailData.orderType * 1 === 14 ? 5 : bigOrder.sourceType,
    ...
    goToTrace(data, type) {
      ...
      if (type == 1) {
        if (this.status < 2 || this.status > 4) return;
    ...
    handleFqzConfirm() {
			...
			this.$wsf.go('Pay', { orderMoney: this.mergeOrderData.orderFqzAmount, orderCode: this.mergeOrderData.masterOrderCode, businessType: 100,
        paySceneType: this.orderDetailData.orderType * 1 === 14 ? 102 : 101 //102集采订单 101电商订单
      });
		},
    // 修改地址
    toaddressList() {
      ...
      if (!this.orderDetailData.orderAttr) {
        return
      }
      ...
      let {
        ...
        receiptName: name,
      } = this.orderDetailData.orderAttr || {}
      // 拼装参数数据
      let params = {
        name,
        mobile,
      ...
      params.orderIdList = (this.orderDetailData.orderDetailVos || []).map(item => {
        return item.id
      })

      this.$wsf.go('Ordereditaddress', {
        ...
        refundStatusBtn (order, bigOrder) {
          if ([1, 3].includes(+order.refundStatus)||(order.refundStatus == 2 && bigOrder.oldRefund != 1) ) {
            ...
            return {
              ...
              name: refundTextMap[order.refundStatus],
              classOther: '',
              func: () => {
                  _that.goRefundServiceDetail(order)
              }
            }
          }
        },
        ...
        addCartBtn (order, bigOrder) {
          ...
          if (bigOrder.status != 1) {
            return {
              type: 'addCartBtn',
              name: '加购物车',
              classOther: 'light-btn',
              func: () => {
                _that.handleAddCart(order,bigOrder)
              }
            }
          }
        }
    ...
    // 按钮点击事件
    skuBtnClick(btnObj) {
      let func = btnObj.func
      func && func()
    },

    contactService(data) {
      this.initContactService(data)
      this.serviceTel()
    },

    // 构造联系客服参数
    initContactService (data) {
      data = data || this.orderDetailData
      let options={}, storeServiceConfig = this.orderDetailData.storeServiceConfig,tagsurl = ''
      Object.keys(storeServiceConfig).forEach(key => {
        ...
        if(options.staffid){
          tagsurl = `https://merchant.wsfmall.com/ordermanagement/mallorder/orderdetail?id=${this.orderDetailData.orderSkuWithMarketings[0].orderId}`
        }else{
          tagsurl = `https://platform.wsfmall.com/ordermagamemt/storeorder/storeorderdetail?id=${this.orderDetailData.orderSkuWithMarketings[0].orderId}`
        }
      let obj = ''
      let ysfData = {
        id:data.id,
        ...
        navigateToUrl:`/orderModule/pages/pendingPayment/pendingPayment?orderId=${data.id}&backStatus=${this.backStatus}`,
        ...options
      }
...
.pending-payment {
  ...
  .content {
    ...
    .header {
      ...
      .go-pay-btn {
        ...
        line-height: 64rpx;
        background: #FFFFFF;
        box-shadow: 0rpx 8rpx 32rpx 0rpx rgba(86,2,16,0.1);
        border-radius: 32rpx;
        font-size: 26rpx;
        font-weight: 400;
        color: #FF0A35;
  ...
  .address-info {
    ...
    .address-info-item:first-of-type {
      margin-top: 0rpx;
    }
    .address-info-logistics {
      width: 100%;
  ...
  .merchandise-news {
    ...
    .sku-remark__desc, .sku-remark__detail {
      font-size: 26rpx;
      font-weight: 400;
      color: #444444;
      line-height: 34rpx;
...
.inform {
  ...
  color: #FF450C;
  border-radius: 16rpx;
  box-shadow: 0px 2px 4px 0px rgba(58,0,12,0.1);
}
/deep/ .u-type-warning-light-bg {
  background: rgba(255, 69, 12, 0.05);
  padding: 12rpx 16rpx 10rpx !important;
  border-radius: 16rpx;
}
/deep/ .u-type-warning {
  font-weight: 500;
...
/deep/ .u-icon__warning::before {
  content: '';
  width: 40px;
  height: 40px;
  background: url(https://wsjc-web-1301582899.cos.ap-guangzhou.myqcloud.com/wsf-mall/order/info-new.png);
  background-size: 100% 100%;
}
