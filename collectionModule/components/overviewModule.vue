<template>
  <view class="overview">
    ...
    <view class="over-img" :class="{'packUp': serveShow}">
			<video style="width: 100%" v-if="spu.video" :poster="spu.videoPic" :src="spu.video"  controls></video>
			<u-parse :html="introduceUrlList"  v-if="spu.detail && spu.detail.length"></u-parse>
			<view v-else-if="spu.mobileImageUrlList && spu.mobileImageUrlList.length" v-for="(item,index) in introduceUrlList" :key="index">
...
export default {
  ...
  computed: {
    introduceUrlList() {
      if (this.spu.detail && this.spu.detail.length) {
        return this.spu.detail.replace(
          /<img /g,
          '<img style="display:block;" '
        );
      } else if (
        this.spu.mobileImageUrlList &&
        this.spu.mobileImageUrlList.length
      ) {
        return this.spu.mobileImageUrlList;
      } else {
        return [];
      }
    },
