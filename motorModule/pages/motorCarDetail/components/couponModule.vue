<template>
  <view>
    <wsf-popup v-model="couponShow" mode="bottom" title="优惠" height="1100">
      <view class="coupon-container">
        <scroll-view scroll-y="true" class="scroll-Y">
          <view class="estimate-module">
            <view class="estimate-card u-margin-bottom-30">
              <view class="price" v-if="selectCouponItem"
                >¥{{ selectCouponItem.afterCouponPrice && selectCouponItem.afterCouponPrice.toFixed(2) }}</view
              >
            ...
            <view class="detail-bubble" v-if="selectCouponItem">
              ...
              <view
                class="detail-bubble-content"
                v-if="selectCouponItem.couponsType != 2"
                >¥{{ selectCouponItem.beforeCouponPrice && selectCouponItem.beforeCouponPrice.toFixed(2) }} - ¥{{
                  selectCouponItem.faceValue
                }}</view
              >
              <view class="detail-bubble-content" v-else
                >¥{{ selectCouponItem.beforeCouponPrice && selectCouponItem.beforeCouponPrice.toFixed(2) }} *
                {{ selectCouponItem.faceValue }}折</view
              >
              <view>
                <view class="estimate-card u-margin-right-64">
                  <view class="price"
                    >¥{{ selectCouponItem.beforeCouponPrice && selectCouponItem.beforeCouponPrice.toFixed(2) }}</view
                  >
                  <view class="text">当前售价</view>
                </view>
                <view class="estimate-card">
                  <view class="price" v-if="selectCouponItem.couponsType == 1"
                    >满{{ selectCouponItem.amountLimitation }}减{{
                      selectCouponItem.faceValue
                    }}</view
                  >
                  <view class="price" v-if="selectCouponItem.couponsType == 2"
                    >{{ selectCouponItem.faceValue }}折</view
                  >
                  <view class="price" v-if="selectCouponItem.couponsType == 3"
                    >满{{ selectCouponItem.faceValue }}减{{
                      selectCouponItem.faceValue
                    }}</view
                  >
          ...
          <view class="title" v-if="usableCouponList && usableCouponList.length"
            >可用</view
          >
            ...
            <block v-for="(item, index) in usableCouponList" :key="index">
              <view class="coupon product-coupon">
                  ...
                  <view class="coupon-detail">
                    ...
                    <view class="coupon-info">
                      <view class="condition" v-if="item.couponsType == 1"
                        >满{{ item.amountLimitation }}可用</view
                      >
                      <view class="condition" v-else-if="item.couponsType == 3"
                        >满{{ item.faceValue }}元可用</view
                      >
                ...
                <view class="stub-ticket">
                  ...
                  ></image>
                  <image
                    src="https://wsjc-web-1301582899.cos.ap-guangzhou.myqcloud.com/wsf-mall/main-packages/activity/cat_ns_platform%402x.png"
                    alt=""
                    class="choosebox"
                    v-else
          ...
          <view
            class="title"
            v-if="disableCouponList && disableCouponList.length > 0"
            >当前不可用</view
          >
        ...
        <view class="coupon-confirm-button" @tap="confirmSelectCoupon()"
          >确定</view
        >
      </view>
    </wsf-popup>
...
export default {
  ...
  methods: {
    ...
    selectCoupon(item) {
		...
		// 	this.selectCouponItem = {}
		// }
		this.selectCouponItem =  item.usable ? item : {}
		// this.selectCouponItem = item;
    },
    confirmSelectCoupon() {
      if (this.selectCouponItem) {
        this.$parent.skuDetail.afterCouponPrice =
          this.selectCouponItem.afterCouponPrice;
      } else {
      ...
      this.$emit("confirmSelectCoupon", this.selectCouponItem);
    },
  },
};
</script>
...
.estimate-module {
  ...
  .detail-bubble {
    ...
    &::before {
      ...
      border-bottom: 20rpx solid #e60113;
    }
    &::after {
      content: "";
      width: 0;
