...
export default {
	...
	onShow: function(options) {
		...
		if (options) {
			...
			if (options.scene === 1037) {
				...
				// vm.$globalData.phone = "15297771614";
	...
	methods: {
		...
		/* 获取模板数据-未使用 大致意思猜测是一定时间重新查询首页模版，然后静默更新模版数据 */
		queryIndexTemplate() {
			this.$u.api.AppletIndex({ storeId: 0 }).then(res => {
				// console.log("res", res);
				if (res.code === 200) {
					// ios 计时器兼容
					res.data.updateTime = res.data.updateTime.replace(/-/g, '/');
					let lastUpdateTime = new Date(res.data.updateTime).getTime();
					if (lastUpdateTime != uni.getStorageSync('_wsf_template_update_time')) {
						// console.log("我要更新模板数据了");
						uni.setStorageSync('_wsf_template', JSON.stringify(res.data));
						uni.setStorageSync('_wsf_template_update_time', JSON.stringify(lastUpdateTime ? lastUpdateTime : new Date().getTime()));
					}
				}
			});
		},
