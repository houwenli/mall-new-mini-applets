<template>
	<view class="spec-module">
		<u-cell-group v-if="false">
			<u-cell-item :title-width="92" title="已选" :arrow="false" :value="specName"
				:title-style="{ 'font-weight': 'bold', color: '#000' }"
				:value-style="{ 'text-align': 'left', color: '#000' }" v-if="
			skuData.sellOut == 1 ||
			skuData.stock == 0 ||
			...
			<u-cell-item :title-width="92" title="已选" :arrow="false" :value="specName"
				:title-style="{ 'font-weight': 'bold', color: '#000' }"
				:value-style="{ 'text-align': 'left', color: '#000' }" @click="openSkuModal(0)" v-else>
				<u-icon slot="right-icon" size="24" name="arrow-right" color="#999"></u-icon>
			</u-cell-item>
			<u-cell-item :title-width="92" title="送至" :arrow="false"
				:title-style="{ 'font-weight': 'bold', color: '#000' }"
				:value-style="{ 'text-align': 'left', color: '#000' }" @click="showAddressModal = true">
				<u-icon slot="right-icon" size="24" name="arrow-right" color="#999"></u-icon>
			...
			<u-cell-item :title-width="92" title="运费" :value="freightFee"
				:title-style="{ 'font-weight': 'bold', color: '#000' }"
				:value-style="{ 'text-align': 'left', color: '#000' }" :arrow="false"></u-cell-item>
			<u-cell-item v-if="
          skuData.spuServiceSupports && skuData.spuServiceSupports.length > 0
        " :title-width="92" title="服务" :title-style="{ 'font-weight': 'bold', color: '#000' }"
				:value-style="{ 'text-align': 'left', color: '#000' }" :arrow="false">
		...
		<view class="service">
			...
			">
				<view class="flex-time">
					<span>已选</span>
				    <view class="selects beyond">{{specName}}</view>
				</view>
			...
			<view class="service-time" v-if="skuData.sku && skuData.sku.goodsSource != 5">
				<view class="flex-time">
					<span>运费</span>
				    <view class="selects">{{!addressList.length && !addressInfo.addressName ? '免运费' : freightFee}}</view>
				</view>
			</view>
		</view>
		...
		<u-popup v-model="specSelectModel" mode="bottom" border-radius="20" height="880rpx" @close="updateSku">
			...
			<view class="popup">
                ...
                activeImg
                  ...
                  ? skuDetail.sku.tree[0].v[0].imgUrl
                  : skuDetail.images[0]
              " @click="goodsimgShow()"></image>
					</view>
					<view class="info">
						<view class="price">
							<view class="txt_item">
								<!-- 添加福气值逻辑 -->
								<view style="display:inline-block" class="fqz-price" v-if="skuData.integralPoint > 0">
									<text>
										<text class="special">
								...
								<template v-if="!skuData.integralPoint">
									<text class="txt_1">￥</text>
									<view class="txt_2"><span>{{ skuPriceInt(skuPrice) }}</span><span class="float">{{
											"." + skuPriceFloat(skuPrice)
										}}</span>
									</view>
								</template>
								
								<view style="display:inline-block" v-if="!bestCoupon && skuDetail.showActiveType && skuDetail.showActiveType[0] && !skuData.afterCouponPrice">
								  <i style="color: #999999; text-decoration: line-through; margin-left: 7px; vertical-align: -webkit-baseline-middle; font-size: 28rpx" v-show="skuDetail.oldPrice > Number(skuPrice)">
										¥{{ skuDetail.oldPrice && skuDetail.oldPrice.toFixed(2) }}
									</i>
								</view>
              </view>
              <view class="performance-mark" v-if="skuDetail.performanceScore && skuData.performanceScore > 0">绩效分<text class="num">{{ skuDetail.performanceScore }}</text></view>
							<view v-if="skuData.afterCouponPrice" class="reduced-price">
                券后￥{{skuData.afterCouponPrice.toFixed(2)}}
              </view>			
              ...
              <view class="sku-val">
                <view
                  ...
                  :class="[
                    ...
                    v.disabled ? 'disable' : '',
                  ]" @click="pitchOnFun(v, treeItem, index_v)" :key="index_v">
									<text class="u-line-1">{{ v.name }}</text>
								</view>
							</view>
						</view>
						<view class="u-flex u-row-between u-padding-top-14 u-padding-bottom-20"
							style="margin-right: 15rpx">
							<b style="font-weight: bold">数量</b>
							<view class="wsf-number-box">
								<text class="skill-txt" v-if="
                    marketingVos.length &&
                    marketingVos[0].type == '13' &&
                    timestamp.label == '距结束还剩'
                  ">限购{{ marketingVos[0].panicBuy.limitNum }}件</text>
								<text class="skill-txt" v-if="
                    marketingVos &&
                    marketingVos.length &&
                    marketingVos.map((item) => item.type).includes('16') &&
                    marketingVos.filter((item) => item.type == '16')[0]
                      .specialArea.limitNum > 0
                  ">限购{{
                    marketingVos.filter((item) => item.type == "16")[0]
                      .specialArea.limitNum
                  }}件</text>
								<view class="minus" @click="minusFun()">
									<u-icon name="minus" :color="
                      addNum !== 1 ? '#000' : addNum == 1 ? '#d8d8d8' : '#000'
                    "></u-icon>
								</view>
								<view class="num">
									<input type="number" v-model="addNum" @input="minusInput($event)"
										@blur="minusBlur($event)" />
								</view>
								<view class="plus" @click="plusFun()">
									<u-icon class="add_icon" name="plus" :color="
                      addNum < limitAddNum && addNum !== limitAddNum
                        ? '#000'
                        : addNum == limitAddNum
                        ? '#d8d8d8'
                        : '#bababa'
				...
				<view class="fixed-column">
					<p v-if="bestCoupon && bestCoupon.type == 1">
						当前商品可使用
						<span>满{{ bestCoupon.couponFull.fullPrice }}减{{
                bestCoupon.couponFull.price
              }}</span>
						店铺优惠券
					</p>
					<view class="spec-btn">
						<!---start 不是虚拟商品不显示加入购物车 skuDetail.isVirtual == 0-->
						<view class="common-btn" :class="{ 'add-cart': canAddCart, 'btn-grey': !canAddCart }" v-if="
                openType != 1 && skuDetail.isVirtual == 0 && !recommendToken && !promoteId
              " @click="handleAddCart">加入购物车</view>
						<!--end 不是虚拟商品不显示加入购物车-->
						<view class="common-btn" :class="{ 'bug-now': canBuyNow, 'btn-grey': !canBuyNow }"
							v-if="openType != 2" @click="handleBugNow">
							{{
	...
	export default {
		...
		watch: {
			...
			'skuData.spuServiceSupports': {
				handler(val, oal) {
					...
					console.log(this.skuData, '123456789')
					setTimeout(() => {
		...
		computed: {
			...
			serviceName() {
				let name = "",
					nameList = [];
				if (
					this.skuData.spuServiceSupports &&
					this.skuData.spuServiceSupports.length > 0
				) {
				...
				if (this.skuData.skuList && this.skuData.skuList.length > 0) {
					this.skuData.skuList.map((item, index) => {
						if (item.stock != 0 && !once) {
							...
							// console.log(this.skuData.sku.tree[0].v[index].imgUrl, 2222);
							// console.log(this.skuData.images[0], 33333);
							...
							url = this.skuData.sku.tree[0].v[index].imgUrl ?
								this.skuData.sku.tree[0].v[index].imgUrl :
								this.skuData.images[0];
						}
					});
				}
				// console.log(url, "url");
				...
				if (this.marketingVos.length && this.marketingVos[0].iSeckill) {
					...
					let obj = {
						buyNowRequestList: [{
							...
							sourceType: this.sourceType,
							storeId: this.skuData.storeId,
							// recommendToken: this.$storage.getItem(this.skuData.spuId), // 2022.9.22 电商v3.0需求取消购物车分销功能, 不缓存分销码
						}, ],
					};
				...
				this.$u.api
					...
					.then((res) => {
						if (res.code == 200) {
							...
							//清除分销码
							// this.$storage.removeItem(this.skuData.spuId); // 2022.9.22 电商v3.0需求取消购物车分销功能, 不缓存分销码
		...
		async CheckSkuPanicBuyLimitNum(data){
			...
			pitchOnFun(v, treeItem, v_i) {
				console.log(v, treeItem, v_i,'wddddddddddd');
                // if(this.initialSkuArr.indexOf(v.id)>-1){
                //     // 点同样的的属性则取消选择状态，其他不变
                //     this.initialSkuArr.splice(this.initialSkuArr.indexOf(v.id),1)
                //     Object.keys(this.initialSku).forEach(item=>{
                //         if(this.initialSku[item] == v.id)this.initialSku[item] =null
                //     } )
                //     if(this.initialSkuArr.length === 0){
                //         //如果没有选择任何属性，则清空属性不可选择状态
				...
				// console.log(v, "v", treeItem, "treeItem");
				// console.log(this.tree[0].v, "this.tree[0].v");
				// console.log(this.skuDetail.sku, "this.skuDetail.sku");
				// console.log(this.skuDetail.sku.tree, "this.skuDetail.sku.tree");
				...
				this.calcSkuDisabled();
                // 判断是否存在有一类属性没有勾选
                let k_sList = this.tree.map(item=>item.k_s)
                let flag = k_sList.some(item=>{
                    return this.initialSku[item] === null
                })
                console.log(flag,'f')
                // 如果有不执行restDetails
                if(flag) return
				this.$emit("restDetails", this.firstOrderSkuId,'',true);
			},
			// 计算Sku选择属性是否可选
			calcSkuDisabled() {
				console.log("计算可选",this.tree);
                this.selectProItem = []
				this.tree.forEach((treeItem) => {
					treeItem.v.forEach((vItem) => {
			...
			filterSku(s, sId) {
				...
				if (this.filterList.length === 1 && this.initialSku[s] == sId) {
					// console.log("找到唯一的skuId了", this.marketingVos);
				...
				this.$u.api.CartSummary({
					...
					if (
						...
						if (this.bestCoupon.type == 1) {
							this.realPrice = (
								this.skuPrice - this.bestCoupon.couponFull.price
							).toFixed(2);
						} else {
      ...
      .sku-item {
        ...
        .sku-val {
          ...
          .pitch-on {
            ...
            background-color: rgba(255, 231, 235, 0.50);
            text {
              color: $wsf-color-new-red;
            }
          }
